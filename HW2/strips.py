...
def linear_solver_helper(world, state, goals, current_plan, previous_states, depth = 0):
    ...
    while i < len(goals):
        ...
        for action in possible_actions:
            ...
            for x in previous_states:
               if x[0] == action and x[1] == list(state) and x[2] == goal:
                    previous_found = True
                    break
            if previous_found:
                continue
            ...
            solution = linear_solver_helper(world, temp_state, subgoals, current_plan, previous_states, depth = depth + 1)
            ...
            # Appending states/actions to not search through
            previous_states.append((action, temp_temp_state, goal))
